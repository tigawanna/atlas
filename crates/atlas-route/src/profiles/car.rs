...
const CAR_SPEEDS: [f64; 8] = [120.0, 100.0, 80.0, 60.0, 50.0, 30.0, 0.0, 0.0];
...
impl RoutingProfile for CarProfile {
    ...
    fn edge_weight(&self, edge: &Edge) -> Option<u32> {
        ...
        let base_speed = CAR_SPEEDS[edge.road_class() as usize];
        if base_speed <= 0.0 {
            return None;
        }
...
mod tests {
    ...
    #[test]
    fn car_rejects_footway() {
        let edge = edge_with(RoadClass::Path, Surface::Paved, false, false);
        assert!(!CarProfile.is_accessible(&edge));
        assert!(CarProfile.edge_weight(&edge).is_none());
    }

    #[test]
    fn car_rejects_track() {
        let edge = edge_with(RoadClass::Track, Surface::Paved, false, false);
        assert!(!CarProfile.is_accessible(&edge));
    }
    ...
    fn unpaved_penalty_applied() {
        ...
        let paved_time = CarProfile.edge_weight(&paved).unwrap();
        let unpaved_time = CarProfile.edge_weight(&unpaved).unwrap();
